...
class Author extends React.Component{
  ...
  render () {
    return (
      <div className="p-t-92 w-960 m-h-auto f-auto p-b-20 animated slideInRight">
        ...
        <div className='m-t-20' style={{width: '990px'}}>
          {
            this.state.authorList.map((item) => (
              <div key={item.id} className='relative w-300 p-h-12 bg-e ta-c p-b-20 border-box br-6 m-r-30 inline-block m-t-60'>
                ...
                <div className='fs-14 c-9 ta-l m-t-16'>最近更新</div>
                { item.article.map((it) => <div key={it.id} className='m-t-20 fs-14 pointer h-b'>{it.title}</div>) }
              </div>
            ))
          }
        </div>
        <div className='ta-c br-20 p-v-10 bg-a pointer m-t-20 c-f w-240 m-h-auto'>加载更多</div>
      </div>
    )
  }
  componentDidMount() {
